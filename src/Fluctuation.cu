...
void cfd::compute_fluctuation(const Block &block, DZone *zone, DParameter *param, int form, Parameter &parameter) {
  ...
  if (problem_type == 1 && form == 1) {
    ...
    ferrer_fluctuation<<<BPG, tpb>>>(zone, param);
  }
}

__global__ void cfd::ferrer_fluctuation(DZone *zone, DParameter *param) {
  ...
  const real eps1 = curand_uniform_double(&(rng(i, j, k, 0))), eps2 = curand_uniform_double(&(rng(i, j, k, 1)));
