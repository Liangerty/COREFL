...
__device__ void hybrid_ep_part(const real *pv, const real *rhoE, const DParameter *param, const real *metric,
  const real *jac, const real *uk, int i_shared, real *fci) {
  constexpr real central_1[3]{0.75, -3.0 / 20, 1.0 / 60};
  const int n_var = param->n_var;
  memset(fci, 0, n_var * sizeof(real));
  for (int l = 1; l <= 3; ++l) {
    real tilde_op[5 + MAX_SPEC_NUMBER + MAX_PASSIVE_SCALAR_NUMBER] = {};
    memset(tilde_op, 0, n_var * sizeof(real));
    for (int m = 0; m < l; ++m) {
      const int is1 = i_shared - m, is2 = i_shared - m + l;
      const auto *pv1 = &pv[is1 * n_var], *pv2 = &pv[is2 * n_var];

      const real weight = 0.125 * (uk[is1] * jac[is1] + uk[is2] * jac[is2]) * (pv1[0] + pv2[0]);
      const real pWeight = 0.25 * (pv1[4] + pv2[4]);

      tilde_op[0] += 2 * weight;
      tilde_op[1] += weight * (pv1[1] + pv2[1]) + pWeight * (metric[is1 * 3] * jac[is1] + metric[is2 * 3] * jac[is2]);
      tilde_op[2] += weight * (pv1[2] + pv2[2]) + pWeight *
          (metric[is1 * 3 + 1] * jac[is1] + metric[is2 * 3 + 1] * jac[is2]);
      tilde_op[3] += weight * (pv1[3] + pv2[3]) + pWeight *
          (metric[is1 * 3 + 2] * jac[is1] + metric[is2 * 3 + 2] * jac[is2]);
      const real Hi = (rhoE[is1] + pv1[4]) / pv1[0];
      const real HiPl = (rhoE[is2] + pv2[4]) / pv2[0];
      tilde_op[4] += weight * (Hi + HiPl);
      for (int n = 0; n < param->n_scalar; ++n) {
        tilde_op[5 + n] += weight * (pv1[5 + n] + pv2[5 + n]);
      }
    }
    const real coefficient = 2 * central_1[l - 1];
    fci[0] += coefficient * tilde_op[0];
    fci[1] += coefficient * tilde_op[1];
    fci[2] += coefficient * tilde_op[2];
    fci[3] += coefficient * tilde_op[3];
    fci[4] += coefficient * tilde_op[4];
    for (int n = 0; n < param->n_scalar; ++n) {
      fci[5 + n] += coefficient * tilde_op[5 + n];
    }
  }
}

__device__ void hybrid_keep_part(const real *pv, const real *rhoE, const DParameter *param, const real *metric,
  const real *jac, const real *uk, int i_shared, real *fci) {
  const int n_var = param->n_var;
  fci[0] = 0;
  fci[1] = 0;
  ...
  fci[4] = 0;
  for (int l = 1; l <= 3; ++l) {
    real tilde_op[5] = {0, 0, 0, 0, 0};
    for (int m = 0; m < l; ++m) {
      const int is1 = i_shared - m, is2 = i_shared - m + l;
      const auto *pv1 = &pv[is1 * n_var], *pv2 = &pv[is2 * n_var];

      const real UK = 0.25 * ((pv1[1] + pv2[1]) * (metric[is1 * 3] * jac[is1] + metric[is2 * 3] * jac[is2]) +
                              (pv1[2] + pv2[2]) * (metric[is1 * 3 + 1] * jac[is1] + metric[is2 * 3 + 1] * jac[is2]) +
                              (pv1[3] + pv2[3]) * (metric[is1 * 3 + 2] * jac[is1] + metric[is2 * 3 + 2] * jac[is2]));
      const real weight = 0.25 * UK * (pv1[0] + pv2[0]);
      const real pWeight = 0.25 * (pv1[4] + pv2[4]);
      ...
      tilde_op[3] += weight * (pv1[3] + pv2[3]) + pWeight *
          (metric[is1 * 3 + 2] * jac[is1] + metric[is2 * 3 + 2] * jac[is2]);
      const real kinetic_energy = weight * (pv1[1] * pv2[1] + pv1[2] * pv2[2] + pv1[3] * pv2[3]);
      const real internal_energy = weight * (pv1[4] / pv1[0] / (1.4 - 1.0) + pv2[4] / pv2[0] / (1.4 - 1.0));
      const real P = 0.25 * (uk[is1] * pv2[4] * jac[is1] + uk[is2] * pv1[4] * jac[is2] +
                             pv1[1] * metric[is2 * 3] * jac[is2] * pv2[4] +
                             pv2[1] * metric[is1 * 3] * jac[is1] * pv1[4] +
                             pv1[2] * metric[is2 * 3 + 1] * jac[is2] * pv2[4] +
                             pv2[2] * metric[is1 * 3 + 1] * jac[is1] * pv1[4] +
                             pv1[3] * metric[is2 * 3 + 2] * jac[is2] * pv2[4] +
                             pv2[3] * metric[is1 * 3 + 2] * jac[is1] * pv1[4]);
      tilde_op[4] += kinetic_energy + internal_energy + P;

      for (int n = 0; n < param->n_scalar; ++n) {
        tilde_op[5 + n] += weight * (pv1[5 + n] + pv2[5 + n]);
      }
    }
    // constexpr real central_1[3]{0.5};
    constexpr real central_1[3]{0.75, -3.0 / 20, 1.0 / 60};
    // constexpr real central_1[4]{0.8, -0.2, 1.0 / 60};
    for (int m = 0; m < 5; ++m) {
      fci[m] += 2.0 * central_1[l - 1] * tilde_op[m];
    }
    for (int n = 0; n < param->n_scalar; ++n) {
      fci[5 + n] += 2.0 * central_1[l - 1] * tilde_op[5 + n];
    }
  }
}

template<MixtureModel mix_model>
__global__ void compute_convective_term_hybrid_weno_ep_x(DZone *zone, DParameter *param) {
  ...
  if (if_shock) {
    ...
    hybrid_keep_part(pv, rhoE, param, metric, jac, uk, i_shared, &fc[tid * n_var]);
    // hybrid_ep_part(pv, rhoE, param, metric, jac, uk, i_shared, &fc[tid * n_var]);
  }
  __syncthreads();
...
template<MixtureModel mix_model>
__global__ void compute_convective_term_hybrid_weno_ep_y(DZone *zone, DParameter *param) {
...
template<MixtureModel mix_model>
__global__ void compute_convective_term_hybrid_weno_ep_z(DZone *zone, DParameter *param) {
  const int i = static_cast<int>(blockDim.x * blockIdx.x + threadIdx.x);
  const int j = static_cast<int>(blockDim.y * blockIdx.y + threadIdx.y);
...
template<MixtureModel mix_model>
void compute_convective_term_hybrid_weno_ep(const Block &block, DZone *zone, DParameter *param, int n_var,
  ...
  compute_convective_term_hybrid_weno_ep_x<mix_model><<<BPG, TPB, shared_mem>>>(zone, param);

  TPB = dim3(1, block_dim, 1);
  BPG = dim3(extent[0], (extent[1] - 1) / (block_dim - 1) + 1, extent[2]);
  compute_convective_term_hybrid_weno_ep_y<mix_model><<<BPG, TPB, shared_mem>>>(zone, param);

  if (extent[2] > 1) {
    TPB = dim3(1, 1, 64);
    BPG = dim3(extent[0], extent[1], (extent[2] - 1) / (64 - 1) + 1);
    compute_convective_term_hybrid_weno_ep_z<mix_model><<<BPG, TPB, shared_mem>>>(zone, param);
  }
}
...
template void compute_convective_term_hybrid_weno_ep<MixtureModel::Air>(const Block &block, DZone *zone,
  DParameter *param, int n_var, const Parameter &parameter);
